...
async function boot({ search = '', config = shipped, configFailure = false, storageBlocked = false,
    empty = false, initialStorage = new Map(), fetchHandler = null, optimizeEnabled = undefined, convertEnabled = false } = {}) {
    class Element {
        constructor() { this.style = {}; this.children = []; this.listeners = {}; this.dataset = {}; this.hidden = false; this._innerHTML = ''; this.classList = { values: new Set(), add(...names) { names.forEach(n => this.values.add(n)); }, remove(...names) { names.forEach(n => this.values.delete(n)); }, toggle(name, force) { const on = force === undefined ? !this.values.has(name) : force; on ? this.values.add(name) : this.values.delete(name); return on; }, contains(name) { return this.values.has(name); } }; }
    ...
    const context = vm.createContext({
        ...
        fetch: async (url, options = {}) => {
            ...
            if (url === '/api/optimize-status') {
                if (optimizeEnabled === undefined) return { ok: false, status: 404, json: async () => ({}) };
                return { ok: true, json: async () => ({ enabled: optimizeEnabled, convert: convertEnabled }) };
            }
...
test('viewer star button toggles, persists to storage, and restores per media', async () => {
    const app = await boot({ search: '?autoplay=1' });
    const icons = { '.star-on-icon': { hidden: true }, '.star-off-icon': { hidden: false } };
    const star = app.get('btn-viewer-star');
    star.querySelector = selector => icons[selector];
    const expectedKey = vm.runInContext('starKeyFor(mediaFiles[0])', app.context);

    await star.listeners.click();
    assert.equal(star.attributes['aria-pressed'], 'true');
    assert.equal(icons['.star-on-icon'].hidden, false);
    assert.equal(icons['.star-off-icon'].hidden, true);
    const stored = JSON.parse(app.saved.get('folderframe.starred'));
    assert.equal(stored.version, 1);
    assert.deepEqual(stored.paths, [expectedKey]);

    // Reopening the media restores the starred (filled) state.
    vm.runInContext('showMedia(0)', app.context);
    assert.equal(icons['.star-on-icon'].hidden, false);
    assert.equal(icons['.star-off-icon'].hidden, true);

    await star.listeners.click();
    assert.equal(star.attributes['aria-pressed'], 'false');
    assert.equal(icons['.star-on-icon'].hidden, true);
    assert.equal(icons['.star-off-icon'].hidden, false);
    assert.equal(JSON.parse(app.saved.get('folderframe.starred')).paths.length, 0);
});

test('grid tile star button toggles a tile and the starred filter keeps only starred media', async () => {
    const app = await boot();
    vm.runInContext(`mediaFiles = [mediaFiles[0], 'https://example.test/frame/photos/second.jpg']; renderGridView();`, app.context);
    const slots = app.get('thumbnail-grid').querySelectorAll('.media-tile-slot');
    assert.equal(slots.length, 2);
    const tileStar = slots[0].children[1];
    assert.equal(tileStar.className, 'tile-star');
    assert.equal(tileStar.attributes['aria-pressed'], 'false');

    tileStar.listeners.click();
    assert.equal(tileStar.attributes['aria-pressed'], 'true');
    assert.equal(JSON.parse(app.saved.get('folderframe.starred')).paths.length, 1);
    assert.equal(JSON.parse(app.saved.get('folderframe.starred')).paths[0],
        vm.runInContext('starKeyFor(mediaFiles[0])', app.context));

    await app.get('btn-stars').listeners.click();
    assert.equal(vm.runInContext('starFilterMode', app.context), true);
    assert.equal(app.get('grid-path').textContent, 'Starred');
    assert.match(app.get('grid-count').textContent, /1 Starred File/);
    assert.equal(app.state().mediaFiles.length, 1);
    const remaining = app.state().mediaFiles[0];
    assert.equal(vm.runInContext(`starKeyFor(${JSON.stringify(remaining)})`, app.context),
        JSON.parse(app.saved.get('folderframe.starred')).paths[0]);

    // Toggling off restores the full listing.
    await app.get('btn-stars').listeners.click();
    assert.equal(vm.runInContext('starFilterMode', app.context), false);
    assert.equal(app.state().mediaFiles.length, 1);
});

test('starred view with no stars shows an empty-state message', async () => {
    const app = await boot();
    await app.get('btn-stars').listeners.click();
    assert.equal(vm.runInContext('starFilterMode', app.context), true);
    assert.equal(app.get('warning-title').textContent, 'No Starred Media');
    assert.equal(app.state().mediaFiles.length, 0);
    await app.get('btn-stars').listeners.click();
    assert.equal(app.state().mediaFiles.length, 1);
});

test('unstarring from the starred view removes the tile immediately', async () => {
    const app = await boot({ initialStorage: new Map([['folderframe.starred', JSON.stringify({ version: 1, paths: ['photo.jpg'] })]]) });
    assert.equal(vm.runInContext('starKeyFor(mediaFiles[0])', app.context), 'photo.jpg');
    await app.get('btn-stars').listeners.click();
    assert.equal(app.state().mediaFiles.length, 1);
    vm.runInContext('renderGridView()', app.context);
    const slot = app.get('thumbnail-grid').querySelectorAll('.media-tile-slot')[0];
    slot.children[1].listeners.click();
    assert.equal(app.state().mediaFiles.length, 0);
    assert.equal(app.get('thumbnail-grid').querySelectorAll('.media-tile-slot').length, 0);
});

test('unstarring from the starred viewer removes the media and advances to the next starred item', async () => {
    const app = await boot({ initialStorage: new Map([['folderframe.starred', JSON.stringify({ version: 1, paths: ['a.jpg', 'b.jpg'] })]]) });
    vm.runInContext("starFilterMode = true; mediaFiles = ['https://example.test/frame/photos/a.jpg', 'https://example.test/frame/photos/b.jpg']; enterFullScreenViewer(0)", app.context);
    assert.equal(vm.runInContext('currentIndex', app.context), 0);
    assert.equal(app.get('btn-viewer-star').attributes['aria-pressed'], 'true');
    await app.get('btn-viewer-star').listeners.click();
    assert.equal(app.state().mediaFiles.length, 1);
    assert.equal(app.state().mediaFiles[0], 'https://example.test/frame/photos/b.jpg');
    assert.equal(vm.runInContext('currentIndex', app.context), 0);
    assert.equal(app.get('btn-viewer-star').attributes['aria-pressed'], 'true');
});

test('unstarring the last starred media from the viewer returns to the empty starred grid', async () => {
    const app = await boot({ initialStorage: new Map([['folderframe.starred', JSON.stringify({ version: 1, paths: ['photo.jpg'] })]]) });
    vm.runInContext("starFilterMode = true; mediaFiles = ['https://example.test/frame/photos/photo.jpg']; enterFullScreenViewer(0)", app.context);
    await app.get('btn-viewer-star').listeners.click();
    assert.equal(app.state().mediaFiles.length, 0);
    assert.equal(app.state().isGridViewActive, true);
});

test('viewer optimize button enables for JPEG/PNG, posts the media-relative path, and restores state', async () => {
    const posted = [];
    const result = { processed: 1, optimized: 1, skipped: 0, bytesBefore: 20480, bytesAfter: 10240, errors: [] };
    const app = await boot({ search: '?autoplay=1', fetchHandler: (url, options = {}) => {
        if (url === '/api/optimize') {
    ...
    const file = app.state().mediaFiles[0];
    assert.equal(file, 'https://example.test/frame/photos/photo.jpg');
    const optimize = app.get('btn-optimize');
    assert.equal(optimize.disabled, false);
    assert.equal(optimize.attributes['aria-label'], 'Optimize photo.jpg');

    await optimize.listeners.click();
    assert.equal(posted.length, 1);
    assert.equal(posted[0].options.method, 'POST');
    assert.deepEqual(JSON.parse(posted[0].options.body), { path: 'photo.jpg' });
    assert.equal(optimize.disabled, false);
    assert.match(app.get('media-loading-text').textContent, /Optimized 1 image/);
    assert.match(app.get('media-loading-text').textContent, /10\.0 KiB saved/);

    // Non-image media keeps the button disabled.
    vm.runInContext("mediaFiles = ['https://example.test/frame/photos/clip.mp4']; updateMediaActions(mediaFiles[0], 'clip.mp4')", app.context);
    assert.equal(optimize.disabled, true);
    assert.match(optimize.title, /JPEG and PNG/);
});

test('folder optimize button is visible and enabled only while browsing a folder', async () => {
    const app = await boot();
    assert.equal(app.get('btn-optimize-folder').hidden, false);
    assert.equal(app.get('btn-optimize-folder').disabled, false);
    vm.runInContext("galleryViewMode = 'all'; updateControlStates(); starFilterMode = true; updateControlStates()", app.context);
    assert.equal(app.get('btn-optimize-folder').hidden, true);
    assert.equal(app.get('btn-optimize-folder').disabled, true);
});

test('folder optimize posts the current folder and refreshes the listing afterwards', async () => {
    const posted = [];
    const result = { processed: 2, optimized: 1, skipped: 1, bytesBefore: 5120, bytesAfter: 4096, errors: [] };
    const app = await boot({ fetchHandler: (url, options = {}) => {
        if (url === '/api/optimize') {
            posted.push({ url, options });
            return { ok: true, status: 200, json: async () => result };
        }
    ...
    await app.get('btn-optimize-folder').listeners.click();
    assert.equal(posted.length, 1);
    assert.deepEqual(JSON.parse(posted[0].options.body), { folder: '' });
    assert.match(app.get('scan-status').textContent, /Optimized 1 image/);
    assert.match(app.get('scan-status').textContent, /1\.0 KiB saved/);
    // loadGallery({ forceCacheClear: true }) re-listed the source folder.
    assert.ok(app.requests.length > requestsBefore);
    assert.ok(app.requests.includes('https://example.test/frame/photos/'));

    // Folder target uses the current album when one is open.
    await boot({ fetchHandler: (url, options = {}) => {
        if (url === '/api/optimize') {
            posted.push({ url, options });
            return { ok: true, status: 200, json: async () => result };
        }
        ...
        vm.runInContext("currentFolder = 'test1'; updateControlStates()", booted.context);
        await booted.get('btn-optimize-folder').listeners.click();
        assert.deepEqual(JSON.parse(posted[posted.length - 1].options.body), { folder: 'test1' });
    });
});

test('optimize failures surface in the grid status and keep the viewer button usable', async () => {
    const app = await boot({ search: '?autoplay=1', fetchHandler: (url) => {
        if (url === '/api/optimize') return { ok: false, status: 404, json: async () => ({ error: 'Optimization is not available' }) };
        return { ok: true, url, text: async () => '' };
    } });
    const optimize = app.get('btn-optimize');
    assert.equal(optimize.disabled, false);
    await optimize.listeners.click();
    assert.equal(optimize.disabled, false);
    assert.match(app.get('scan-status').textContent, /Optimize failed/);
});

test('optimize buttons reflect the deployment capability endpoint', async () => {
    const disabled = await boot({ search: '?autoplay=1', optimizeEnabled: false });
    assert.equal(disabled.get('btn-optimize').disabled, true);
    assert.match(disabled.get('btn-optimize').title, /disabled/);
    vm.runInContext("galleryViewMode = 'folders'; updateControlStates()", disabled.context);
    assert.equal(disabled.get('btn-optimize-folder').disabled, true);
    assert.match(disabled.get('btn-optimize-folder').title, /disabled/);

    const enabled = await boot({ search: '?autoplay=1', optimizeEnabled: true });
    assert.equal(enabled.get('btn-optimize').disabled, false);
    vm.runInContext("galleryViewMode = 'folders'; updateControlStates()", enabled.context);
    assert.equal(enabled.get('btn-optimize-folder').disabled, false);
});

test('optimize buttons stay usable when the deployment has no capability endpoint', async () => {
    const app = await boot({ search: '?autoplay=1' });
    assert.equal(app.get('btn-optimize').disabled, false);
    vm.runInContext("galleryViewMode = 'folders'; updateControlStates()", app.context);
    assert.equal(app.get('btn-optimize-folder').disabled, false);
});

test('optimize tooltips do not mention commands or use parentheses', async () => {
    const app = await boot({ search: '?autoplay=1', optimizeEnabled: true });
    const viewer = app.get('btn-optimize');
    assert.equal(viewer.title, 'Optimize this image');
    assert.doesNotMatch(viewer.title, /[()]|jpegoptim|oxipng/);
    vm.runInContext("galleryViewMode = 'folders'; updateControlStates()", app.context);
    const folder = app.get('btn-optimize-folder');
    assert.equal(folder.title, 'Optimize every JPEG and PNG directly in this folder');
    assert.doesNotMatch(folder.title, /[()]|jpegoptim|oxipng|subfolders/);
});

test('optimize shows the scan progress bar with busy indicator and hides it afterwards', async () => {
    let resolveFetch;
    const result = { processed: 2, optimized: 1, skipped: 1, bytesBefore: 2048, bytesAfter: 1024, errors: [] };
    const app = await boot({ search: '?autoplay=1', fetchHandler: (url, options = {}) => {
        if (url === '/api/optimize') return new Promise(resolve => { resolveFetch = resolve; });
        return { ok: true, url, text: async () => '' };
    } });
    const track = app.get('scan-progress-track');
    const fill = app.get('scan-progress-fill');
    const loading = app.get('scan-loading');
    assert.equal(track.hidden, true);
    assert.equal(loading.hidden, true);
    const clickPromise = app.get('btn-optimize').listeners.click();
    // flush microtasks so the handler reaches its pending fetch
    for (let i = 0; i < 20; i++) await Promise.resolve();
    assert.equal(loading.hidden, false);
    assert.equal(track.hidden, false);
    assert.equal(fill.classList.contains('is-indeterminate'), true);
...
test('viewer convert opens the format prompt and posts the chosen type after confirming', async () => {
    const posted = [];
    const result = { processed: 1, converted: 1, skipped: 0, errors: [], format: 'jpg' };
    const app = await boot({ search: '?autoplay=1', optimizeEnabled: true, convertEnabled: true, fetchHandler: (url, options = {}) => {
        if (url === '/api/convert') {
            posted.push({ url, options });
            return { ok: true, status: 200, json: async () => result };
        }
    ...
    const file = app.state().mediaFiles[0];
    assert.equal(file, 'https://example.test/frame/photos/photo.jpg');
    const convert = app.get('btn-convert');
    assert.equal(convert.disabled, false);
    assert.equal(convert.attributes['aria-label'], 'Convert photo.jpg');

    await convert.listeners.click();
    assert.equal(app.get('convert-overlay').style.display, 'flex');
    assert.equal(posted.length, 0); // nothing posted until confirmed

    app.get('convert-format').value = 'jpg';
    await app.get('btn-convert-confirm').listeners.click();
    assert.equal(app.get('convert-overlay').style.display, 'none');
    assert.equal(posted.length, 1);
    assert.equal(posted[0].options.method, 'POST');
    assert.deepEqual(JSON.parse(posted[0].options.body), { path: 'photo.jpg', format: 'jpg' });
    assert.equal(convert.disabled, false);
    assert.match(app.get('media-loading-text').textContent, /Converted 1 image to JPEG/);

    // Cancelling the prompt never sends a request.
    await convert.listeners.click();
    app.get('btn-convert-cancel').listeners.click();
    assert.equal(app.get('convert-overlay').style.display, 'none');
    assert.equal(posted.length, 1);
});

test('folder convert is visible only while browsing a folder and posts folder plus format', async () => {
    const posted = [];
    const result = { processed: 2, converted: 1, skipped: 1, errors: [], format: 'webp' };
    const app = await boot({ optimizeEnabled: true, convertEnabled: true, fetchHandler: (url, options = {}) => {
        if (url === '/api/convert') {
            posted.push({ url, options });
            return { ok: true, status: 200, json: async () => result };
        }
    ...
    assert.equal(app.get('btn-convert-folder').hidden, false);
    assert.equal(app.get('btn-convert-folder').disabled, false);
    vm.runInContext("galleryViewMode = 'all'; updateControlStates(); starFilterMode = true; updateControlStates()", app.context);
    assert.equal(app.get('btn-convert-folder').hidden, true);
    assert.equal(app.get('btn-convert-folder').disabled, true);
    vm.runInContext("galleryViewMode = 'folders'; starFilterMode = false; updateControlStates()", app.context);

    const requestsBefore = app.requests.length;
    app.get('btn-convert-folder').listeners.click();
    app.get('convert-format').value = 'webp';
    await app.get('btn-convert-confirm').listeners.click();
    assert.equal(posted.length, 1);
    assert.deepEqual(JSON.parse(posted[0].options.body), { folder: '', format: 'webp' });
    assert.match(app.get('scan-status').textContent, /Converted 1 image to WebP/);
    assert.ok(app.requests.length > requestsBefore); // reloaded the listing

    // Folder target uses the current album when one is open.
    await boot({ optimizeEnabled: true, convertEnabled: true, fetchHandler: (url, options = {}) => {
        if (url === '/api/convert') {
            posted.push({ url, options });
            return { ok: true, status: 200, json: async () => result };
        }
        ...
        vm.runInContext("currentFolder = 'test1'; updateControlStates()", booted.context);
        booted.get('btn-convert-folder').listeners.click();
        booted.get('convert-format').value = 'webp';
        await booted.get('btn-convert-confirm').listeners.click();
        const body = JSON.parse(posted[posted.length - 1].options.body);
        assert.equal(body.folder, 'test1');
        assert.equal(body.format, 'webp');
    });
});

test('convert buttons reflect the deployment capability endpoint', async () => {
    const disabled = await boot({ search: '?autoplay=1', optimizeEnabled: true, convertEnabled: false });
    const viewer = disabled.get('btn-convert');
    assert.equal(viewer.disabled, true);
    assert.match(viewer.title, /disabled/);
    vm.runInContext("galleryViewMode = 'folders'; updateControlStates()", disabled.context);
    assert.equal(disabled.get('btn-convert-folder').disabled, true);
    assert.match(disabled.get('btn-convert-folder').title, /disabled/);

    const enabled = await boot({ search: '?autoplay=1', optimizeEnabled: true, convertEnabled: true });
    assert.equal(enabled.get('btn-convert').disabled, false);
});

test('convert tooltips do not mention commands or use parentheses', async () => {
    const app = await boot({ search: '?autoplay=1', optimizeEnabled: true, convertEnabled: true });
    const viewer = app.get('btn-convert');
    assert.equal(viewer.title, 'Convert this image to another type');
    assert.doesNotMatch(viewer.title, /[()]|mogrify|ImageMagick/);
    vm.runInContext("galleryViewMode = 'folders'; updateControlStates()", app.context);
    const folder = app.get('btn-convert-folder');
    assert.equal(folder.title, 'Convert images in this folder to another format');
    assert.doesNotMatch(folder.title, /[()]|mogrify|ImageMagick|subfolders/);
});

test('convert shows the scan progress bar with busy indicator and hides it afterwards', async () => {
    let resolveFetch;
    const result = { processed: 1, converted: 1, skipped: 0, errors: [], format: 'png' };
    const app = await boot({ search: '?autoplay=1', optimizeEnabled: true, convertEnabled: true, fetchHandler: (url, options = {}) => {
        if (url === '/api/convert') return new Promise(resolve => { resolveFetch = resolve; });
        return { ok: true, url, text: async () => '' };
    } });
    const track = app.get('scan-progress-track');
    const fill = app.get('scan-progress-fill');
    ...
    assert.equal(track.hidden, true);
    assert.equal(loading.hidden, true);
    app.get('convert-format').value = 'png';
    app.get('btn-convert').listeners.click();
    app.get('btn-convert-confirm').listeners.click();
    // flush microtasks so the handler reaches its pending fetch
    for (let i = 0; i < 20; i++) await Promise.resolve();
    assert.equal(loading.hidden, false);
    assert.equal(track.hidden, false);
...
test('convert delete-original checkbox sends delete and reports deleted count', async () => {
    const posted = [];
    const result = { processed: 1, converted: 1, skipped: 0, deleted: 1, errors: [], format: 'webp' };
    const app = await boot({ search: '?autoplay=1', optimizeEnabled: true, convertEnabled: true, fetchHandler: (url, options = {}) => {
        if (url === '/api/convert') {
            posted.push({ url, options });
            return { ok: true, status: 200, json: async () => result };
    ...
    app.get('convert-format').value = 'webp';
    app.get('convert-delete-original').checked = false;
    app.get('btn-convert').listeners.click();
    await app.get('btn-convert-confirm').listeners.click();
    assert.equal(posted.length, 1);
    assert.equal(JSON.parse(posted[0].options.body).delete, undefined);
    // A successful single-file convert refreshes the listing; the mock scan
    // returns an empty folder, so restore the playlist before converting again.
    vm.runInContext("mediaFiles = ['https://example.test/frame/photos/photo.jpg']; currentIndex = 0", app.context);

    app.get('btn-convert').listeners.click();
    app.get('convert-delete-original').checked = true;
    await app.get('btn-convert-confirm').listeners.click();
    assert.equal(posted.length, 2);
    assert.equal(JSON.parse(posted[1].options.body).delete, true);
    assert.match(app.get('media-loading-text').textContent, /1 original deleted/);
});

test('convert cannot be started again while a conversion is already running', async () => {
    let resolveConvert;
    const app = await boot({ search: '?autoplay=1', optimizeEnabled: true, convertEnabled: true, fetchHandler: (url, options = {}) => {
        if (url === '/api/convert') return new Promise(resolve => { resolveConvert = resolve; });
        return { ok: true, url, text: async () => '' };
    } });
    app.get('btn-convert-folder').listeners.click();
    app.get('convert-format').value = 'webp';
    app.get('btn-convert-confirm').listeners.click();
    for (let i = 0; i < 20; i++) await Promise.resolve();
    assert.equal(app.get('convert-overlay').style.display, 'none', 'prompt closed once the run started');

    // While the convert is in flight neither the folder nor the viewer button
    // may open the prompt again (the server would reject the extra job with 409).
    app.get('btn-convert-folder').listeners.click();
    app.get('btn-convert').listeners.click();
    for (let i = 0; i < 20; i++) await Promise.resolve();
    assert.equal(app.get('convert-overlay').style.display, 'none', 'no second prompt while a convert runs');

    resolveConvert({ ok: true, status: 200, json: async () => ({ processed: 1, converted: 1, skipped: 0, deleted: 0, errors: [], format: 'webp' }) });
    for (let i = 0; i < 40; i++) await Promise.resolve();
    assert.equal(app.get('convert-overlay').style.display, 'none');
});

test('single-file convert refreshes the listing so the new file appears without a manual reload', async () => {
    const result = { processed: 1, converted: 1, skipped: 0, deleted: 0, errors: [], format: 'jpg' };
    const app = await boot({ search: '?autoplay=1', optimizeEnabled: true, convertEnabled: true, fetchHandler: (url, options = {}) => {
        if (url === '/api/convert') return { ok: true, status: 200, json: async () => result };
        return { ok: true, url, text: async () => '' };
    } });
    const marker = app.requests.length;
    app.get('convert-format').value = 'jpg';
    app.get('btn-convert').listeners.click();
    await app.get('btn-convert-confirm').listeners.click();
    for (let i = 0; i < 40; i++) await Promise.resolve();
    assert.ok(app.requests.slice(marker).some(url => String(url).includes('/photos/')),
        `expected a gallery listing refresh after single-file convert; requests=${JSON.stringify(app.requests.slice(marker))}`);
    assert.equal(app.get('scan-loading').hidden, true);
    assert.equal(vm.runInContext('operationInFlight', app.context), false);
});

test('auto-refresh is suspended while a convert runs so the busy indicator stays visible', async () => {
    let resolveConvert;
    const app = await boot({ search: '?autoplay=1', optimizeEnabled: true, convertEnabled: true, fetchHandler: (url, options = {}) => {
        if (url === '/api/convert') return new Promise(resolve => { resolveConvert = resolve; });
        return { ok: true, url, text: async () => '' };
    ...
    vm.runInContext('startAutoRefreshTimer()', app.context);
    assert.ok(tick, 'auto-refresh timer callback must be captured');

    app.get('btn-convert-folder').listeners.click();
    app.get('convert-format').value = 'webp';
    app.get('btn-convert-confirm').listeners.click();
    for (let i = 0; i < 20; i++) await Promise.resolve();
    assert.equal(app.get('scan-loading').hidden, false, 'busy indicator visible while converting');

    const baseline = app.requests.length;
    tick(); // an auto-refresh tick fires while the convert is still in flight
    for (let i = 0; i < 20; i++) await Promise.resolve();
    assert.equal(app.requests.length, baseline, 'auto-refresh must not rescan while a convert is in flight');
    assert.equal(app.get('scan-loading').hidden, false, 'busy indicator still visible after the suppressed tick');

    resolveConvert({ ok: true, status: 200, json: async () => ({ processed: 1, converted: 1, skipped: 0, deleted: 0, errors: [], format: 'webp' }) });
    for (let i = 0; i < 20; i++) await Promise.resolve();
    assert.ok(app.requests.length > baseline, 'completion refresh still runs after the convert finishes');
    assert.equal(app.get('scan-loading').hidden, true, 'indicator hidden when done');
    assert.equal(vm.runInContext('operationInFlight', app.context), false);
});
